            ...
            if (session.payment_status === "paid") {
                ...
                if (userId && itemsRaw) {
                    ...
                    if (parsedOrder.success) {
                        const userResult = await pool.query(`SELECT id, email, full_name AS "fullName", role
               FROM users
               WHERE id = $1
               LIMIT 1`, [userId]);
...
async function createOrderForUser(user, items, stripeSessionId) {
    const client = await pool.connect();
    try {
        await client.query("BEGIN");
        if (stripeSessionId) {
            const existing = await client.query("SELECT id FROM orders WHERE stripe_session_id = $1 LIMIT 1", [stripeSessionId]);
        ...
        const orderInsert = await client.query(`INSERT INTO orders (user_id, customer_name, customer_email, stripe_session_id)
       VALUES ($1, $2, $3, $4)
       RETURNING id`, [user.id, customerName, user.email, stripeSessionId ?? null]);
...
app.post("/api/payments/checkout-session", requireAuth, async (req, res) => {
    ...
    try {
        ...
        const productsResult = await pool.query(`SELECT
        id,
        name,
        description,
        image_url AS "imageUrl",
        ...
        is_sold_out AS "isSoldOut"
       FROM products
       WHERE id = ANY($1::text[])`, [productIds]);
